...
public class SysDeptController extends BaseController
{
    ...
    public AjaxResult add(@Validated @RequestBody SysDept dept)
    {
        ...
        {
            return AjaxResult.error("新增学校'" + dept.getDeptName() + "'失败，学校名称已存在");
        }
    ...
    public AjaxResult edit(@Validated @RequestBody SysDept dept)
    {
        ...
        if (UserConstants.NOT_UNIQUE.equals(deptService.checkDeptNameUnique(dept)))
        {
            return AjaxResult.error("修改学校'" + dept.getDeptName() + "'失败，学校名称已存在");
        }
        else if (dept.getParentId().equals(deptId))
        {
            return AjaxResult.error("修改学校'" + dept.getDeptName() + "'失败，上级学校不能是自己");
        }
        else if (StringUtils.equals(UserConstants.DEPT_DISABLE, dept.getStatus()) && deptService.selectNormalChildrenDeptById(deptId) > 0)
        {
            return AjaxResult.error("该学校包含未停用的子学校！");
        }
    ...
    public AjaxResult remove(@PathVariable Long deptId)
    {
        ...
        {
            return AjaxResult.error("存在下级学校,不允许删除");
        }
        if (deptService.checkDeptExistUser(deptId))
        {
            return AjaxResult.error("学校存在用户,不允许删除");
        }
        deptService.checkDeptDataScope(deptId);
